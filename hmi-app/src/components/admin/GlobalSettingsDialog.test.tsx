...
vi.mock('./DesignSettingsTab', async () => {
    ...
    return {
        default: function MockDesignSettingsTab({ onDirtyChange, onSaveStatusChange, saveRef, revertRef }: { onDirtyChange?: (dirty: boolean) => void; onSaveStatusChange?: (status: 'dirty' | 'saving' | 'saved' | 'error' | null) => void; saveRef?: { current: (() => void) | null }; revertRef?: { current: (() => void) | null } }) {
...
vi.mock('./LoaderOptionsSettingsTab', async () => {
    ...
    return {
        default: function MockLoaderOptionsSettingsTab({ onDirtyChange, onSaveStatusChange, saveRef }: { onDirtyChange?: (dirty: boolean) => void; onSaveStatusChange?: (status: 'dirty' | 'saving' | 'saved' | 'error' | null) => void; saveRef?: { current: (() => void) | null } }) {
...
describe('GlobalSettingsDialog', () => {
    ...
    it('projects the Diseno save status in the footer before Guardar, per tab, and clears it on close', async () => {
        const user = userEvent.setup();
        render(<Harness />);

        await user.click(screen.getByRole('button', { name: 'Diseno' }));
        await user.type(screen.getByLabelText('Design draft'), 'X');
    ...
    it('projects the Opciones save status in the footer before Guardar, per tab, and clears it on close', async () => {
        ...
        await user.click(screen.getByRole('button', { name: 'Opciones' }));
        await user.type(screen.getByLabelText('Loader draft'), 'X');
