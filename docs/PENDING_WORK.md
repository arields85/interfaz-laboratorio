...
| PW-003 | pending | Prisma Assistant — Canal A | Composición bot A → vinculación → contexto de la HMI correcta → respuesta aceptada offline con 10 pruebas; aún sin conectar al arranque. RCA-5 sigue incompleto: orden/invalidez de contexto, proyección de vínculo, política/admin, scheduler/proxy/bootstrap y aceptación más amplia de identidad. QR futuro solo por clic en Prisma junto a Logs. Error del servicio local bloquea credenciales, causa pendiente. Arneses auxiliares pausados. Ver `odd/tasks/prisma-channel-a-remote.md`. | `backlog/prisma-dual-channel-assistant` | 2026-09-17 |
